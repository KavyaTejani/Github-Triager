...
def clamp_score(score: float) -> float:
    """Strictly maps [0, 1] to [0.01, 0.99]."""
    return round(0.01 + (max(0.0, min(1.0, score)) * 0.98), 4)

def recursive_clamp(data: Any) -> Any:
    """
    Recursively clamps ALL numbers (float/int) to (0.01, 0.99).
    This ensures no '0' or '1' exists in any field (reward, info, obs).
    """
    if isinstance(data, bool): # Bools are ints in Python, skip them
        return data
...
class FullTriageGrader:
    ...
    def grade(self, action: FullTriageAction, gold: Dict) -> FullTriageReward:
        ...
        assignee_match = (action.suggested_assignee == gold.get('gold_assignee'))
        breakdown['assignee'] = 0.15 if assignee_match else 0.0
        comp_match = (action.suggested_component == gold.get('gold_component'))
        breakdown['component'] = 0.15 if comp_match else 0.0
        ...
        return FullTriageReward(
            ...
            assignee_correct=assignee_match,
            component_correct=comp_match,
            breakdown=breakdown
        )
...
class BatchTriageGrader:
    ...
    def grade_step(self, action: BatchTriageAction, gold: Dict) -> BatchTriageReward:
        ...
        raw_step = (base_reward.score - 0.01) / 0.98 + dup_bonus
        ...
        return BatchTriageReward(
            ...
            trajectory_score=0.01, duplicate_detection_bonus=dup_bonus,
            breakdown=base_reward.breakdown, is_trajectory_final=False
        )

    def grade_trajectory(self) -> BatchTriageReward:
        total_step = 0.0
        n = len(self.trajectory_actions)
        if n == 0: return BatchTriageReward(score=0.01, step_score=0.01, trajectory_score=0.01, is_trajectory_final=True)
        ...
        final_traj = clamp_score(avg_step) # Simplified for total robustness
        return BatchTriageReward(
            score=final_traj, step_score=clamp_score(avg_step),
            trajectory_score=final_traj, is_trajectory_final=True,
